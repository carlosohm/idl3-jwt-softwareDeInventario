    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
            <b-row>
                <b-col md="10">
                    <strong>Modulo Tipo de Usuario | Nuevo</strong>
                </b-col>
                <b-col md="2">
...
function GetZonePrivileges() {
    let me = this;
    let url = this.url_base + "user-type/get-zones-privileges"
    axios({
        method: "GET",
        url: url,
        ...
        if (response.data.status == 200) {
            me.zone_privileges = response.data.result;
        }
    }).catch((error) => {
...
function AddUserType() {
    ...
    axios({
        ...
        if (response.data.status == 201) {
            me.user_type.id_user_type = '';
            me.user_type.name = '';
            me.user_type.state = '1';
            Swal.fire({ icon: 'success', text: response.data.message, timer: 3000})
            me.$router.push({name: "UserTypeList" });
...
function Validate() {
    ...
    if (this.validate) {
     ...
     Swal.fire({
      title: "Esta seguro de registrar el tipo de usuario?",
      text: "",
      icon: "warning",
